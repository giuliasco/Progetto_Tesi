...
{
    //bits 0-3: dimensione dell'albero, esclusa la radice
    //bits 4-7: dimensione del sottoalbero radicatp nell'ultimo figlio della radice
    //bits 8-11: numero di copie del sottoalbero radicato nell'ultimo figlio.
    //bits 12-27: maschera dei colori
    //bits 28-57: risultato della visita DFS dell'albero (struttura dell'albero)


    /*
    Creazione dell'albero formato da un unico elemento colorato
     */
...
*/
    ...
    {
        int size1 = (int)(t1 & 0xFL); //restituiscono le dimensioni di t1 e t2 rispettivamente
        int size2 = (int)(t2 & 0xFL);

        long structure1 = (t1>>28) & 0xFFFFFFFFL; //restituiscono le strutture di t1 e t2 rispettivamente
        long structure2 = (t2>>28) & 0xFFFFFFFFL;

        long ncopies = 1;

        //si verifica la proprietà (1)
        if(size1!=0)
        {
        ...
        //si verifica la proprietà (2)
        if( (t1 & t2 & 0xFFFF000L)!=0 )
            return -1;

        // si genera il nuovo albero t a partire dalla struttura
    ...
    //fattore di normalizzazine per unioni non bilanciate
    public static int normalization_factor(long t)
    {
        return (int)((t>>8) & 0xFL);
    }

    //fattore di normalizzazine per unioni bilanciate
    ...
    public static long reroot(long structure, int v, StructureInfo info)
    {
        ...
        int path[] = new int[16]; //Indice dei vertici nel percorso da v alla radice t
        int path_length = 0;
        do 
        {
            path[path_length++]=v;
        ...
        //Da questo punto avviene il vero e proprio riradicamento
    ...
    //Appendde un albero codificato dai primi n bit di s come un figlio della radice dell'albero t
    ...
    {        
        ...
        for(int i=0; i<info.number_of_vertices; i++)
        {
            ...
            {
                int size = (info.structure_start[u] - info.structure_end[u] + 1)/2 + 1; //numero dei vertici nel sottoalbero di s radicato in u
...
class StructureInfo
{
    ...
    {        
        int size = Long.bitCount(structure); //Numero di archi nella struttura
        ...
        int current=0; //L'indice dei vertici che stiamo visitando al momento
        int last_vertex=0; //L'indice dell'ultimo vertice scoperto

        for(int i=0; i<2*size; i++)
        {
            if( ((structure>>(2*size-1-i)) & 1L) != 0)
            {
                //nuovo arco diretto lontano dalla radice
            ...
            {
                //Archi che ritornano verso il nodo genitore
