...
\begin{algorithm}[H]
	...
	\For{$ i = 1,\dots,\ell $}
	{
		\If{$ \sum_{j=1}^{i}{|T_j|} > \frac{2}{3}\cdot S $ \label{ln:if_alg2}} 
		{
...
\begin{algorithm}[H]
	\label{algoritmo2}
	\SetAlgoLined
	\caption{Algoritmo per il conteggio del numero di occorrenze di $ k $-treelet ben-colorati in $ G $, usando le decomposizioni  $ (\lfloor \frac{2}{3}(k-1) \rfloor + 1)$-bilanciate}
	\textbf{input} : Grafo $ G =(V,E) $, dimensione deI treelet di interesse $ k \ge 3 $;\\		
	\For{$ h = 1$ to $ \left\lfloor \frac{2}{3}(k-1) \right\rfloor +1 $}{
	Si calcolano le occorrenze seguendo l'Algoritmo \ref{algoritmo} riportato in  sezione \ref{section1};\\
	}
	...
	\ForEach{$ T : |T| = k $}{
		...
		\If{$ c \ != \ v $}
		 {\textbf{continue}; } 
		Suddivido $ T $ in due alberi $ T' $ e $ T'' $ come descritto in precedenza (sezione \ref{section1})\; 
			\[c(T_C,v) = \frac{1}{\gamma_T}\sum_{\substack{
C',C'' \subseteq C : |C'| = |T'| \\
C' \cup C'' = C \\ 
C' \cap C'' = \{c_r\}}}
...
Come nell'algortimo \ref{algoritmo} il numero di occorrenze di $ T $ è calcolato con un approccio basato sulla decomposizione di $T$ in due sottoalberi $T', T''$. Anche in questo caso, come nel \ref{cap 2}, si è sfruttato l'approccio inverso, basato cioè, sull'ottenere $T$ come unione di $T'$ a $T''$.\\
